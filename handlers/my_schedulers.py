...
# def get_all_users():
#     cursor.execute('SELECT id FROM timetable WHERE time_notif != "-"')
...
#     with open(json_name, 'r') as file:
#         data = json.load(file)
#     today_data = datetime.today().date()  # Изменено
#     list_tomorrow = []
#     for tmp in data:
#         if (tmp['Группа'].lower() == gr_name.lower() and int(tmp['ВремяНачала'].partition('.')[0]) == ((((today_data-client.first_odd_week).days)%14)+2)):
#             list_tomorrow.append(tmp)
...
#         try:
#             message = await tomorrow_tb_notif(user_id)
#             await bot.send_message(user_id, message)  # Исправлено
#         except Exception as e:
#             logging.error(f"Не удалось отправить сообщение пользователю {user_id}: {e}")

# def schedule_jobs():
#     scheduler.add_job(send_message_to_all, "interval", seconds=5)  # Исправлено
#     scheduler.start()
...
#         print(user_id)
#         try:
#             await bot.send_message(user_id, tomorrow_tb_notif(user_id))
#         except Exception as e:
#             logging.error(f"Не удалось отправить сообщение пользователю {user_id}: {e}")

# def schedule_jobs():
#     scheduler.add_job(send_message_to_all,"interval", seconds=5, args=(dp,))
#     scheduler.start()

        

# async def send_tb_time(dp: Dispatcher):
#     await bot.send_message(707726833, "hello xxxxx")

# def schedule_jobs():
#     scheduler.add_job(send_tb_time, "interval", seconds=5, args=(dp,)) 
